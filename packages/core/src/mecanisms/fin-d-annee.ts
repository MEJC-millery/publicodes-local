...
export type FinDAnnéeNode = {
	explanation: ASTNode
	nodeKind: "fin d'année"
}

const evaluate: EvaluationFunction<"fin d'année"> = function (node) {
	const valeur = this.evaluateNode(node.explanation)
	let nodeValue = valeur.nodeValue
	if (nodeValue !== null)
	{
		if (typeof nodeValue === "number")
			nodeValue = `31/12/${nodeValue}`
...
registerEvaluationFunction("fin d'année", evaluate)

export default function parseFinDAnnée(v, context) {
...
parseFinDAnnée.nom = "fin d'année"
